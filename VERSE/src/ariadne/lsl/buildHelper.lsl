...
string      HTTPDB_URL          = "http://142.51.75.111/SLDataCollection/builder/obj.aspx";
...
httpdb_request( integer type, string type_str, string name, string body ) {
    key reqid = llHTTPRequest(HTTPDB_URL, [HTTP_METHOD, "POST"], "?cmd=" + type_str + "&" + name + "=" + llEscapeURL(body));
    if ( reqid == NULL_KEY )
...
// Delete a value from httpdb, then insert its replacement.
httpdb_update( string name, string value ) {
	httpdb_delete(name);
	httpdb_save(name,value);
}
...
httpdb_success(integer type, string name, integer status, string body) {
    ...
    llMessageLinked(LINK_SET, HTTPDB_SUCCESS, llList2CSV([type, status, body]), name );
     llSay(0, "success:"+ name +"="+llList2CSV([type, status, body]));
